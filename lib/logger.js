...
class Logger extends Map {
   ...
   * @param {Boolean} [doubleWrite] - whether write to both logger
   */
  redirect(level, logger, doubleWrite) {
    level = level.toUpperCase();
    if (!this.redirectLoggers.has(level) && logger instanceof Logger) {
      this.redirectLoggers.set(level, { logger, doubleWrite });
    }
  }
